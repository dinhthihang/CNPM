...
public class KhachHangData {
    ...
    public boolean UpdateKhachHang(KhachHang kh) {
        try {
            ps = Connect.getConnect().prepareStatement("UPDATE Ban_Doc SET Password = ?, Ten_Ban_Doc = ?,"
    ...
    public boolean DeleteKhachHang(String maKH) {
        try {
            ps = Connect.getConnect().prepareStatement("DELETE FROM Ban_Doc WHERE Ma_Ban_Doc = ?");
            ps.setString(1, maKH);
            return ps.executeUpdate() >0;
        } catch(Exception e) {
            return false;
